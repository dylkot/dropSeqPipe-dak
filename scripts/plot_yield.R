...
#  reads_after_filtering = as.numeric(str_match(bbmap_log['Pairs',], pattern = "\t([0-9]{1,20}) reads.*")[,2])/2
  R1_filtered = read.table(snakemake@input$R1_filtered[i], header = FALSE, skip=11, sep=':', nrows=7, row.names=1)
  total_reads = as.numeric(str_remove_all(R1_filtered['Total read pairs processed',], pattern = (' |,')))
  reads_after_filtering = as.numeric(str_remove_all(sub('\\(.*','',R1_filtered['Pairs written (passing filters)',]), pattern = (' |,')))
