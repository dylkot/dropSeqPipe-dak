...
rule extract_umi_expression:
    ...
    resources:
        mem_gb=int(config['LOCAL']['memory'][:-1])
    shell:
...
rule extract_UMI_soup_profile:
    input:
        data='{results_dir}/samples/{sample}/unassignedcbs.bam',
        barcode_whitelist='{results_dir}/samples/{sample}/genericbarcode.txt'
    output:
        long='{results_dir}/samples/{sample}/umi/unassignedcbs.long',
        dense=temp('{results_dir}/samples/{sample}/umi/unassignedcbs.tsv')
    params:
        count_per_umi=config['EXTRACTION']['minimum-counts-per-UMI'],
        num_cells=lambda wildcards: int(samples.loc[wildcards.sample,'expected_cells']),
        umiBarcodeEditDistance=config['EXTRACTION']['UMI-edit-distance'],
